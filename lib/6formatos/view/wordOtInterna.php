...
//DATOS ENCABEZADO //
$table->addRow();
$table->addCell(4000, array('vMerge' => 'restart', 'valign' => 'center'))->addImage('../../../img/logo.jpg', array('width' => 85, 'height' => 40, 'align' => 'center', 'valign' => 'center'));
...
$table_des->addRow(1);
// Add cells
$table_des->addCell(2000, $styleCell)->addText('No. ORDEN INTERNA', $fontStyle, 'p2Style');
$table_des->addCell(2800, $styleCell)->addText(utf8_decode('FECHA DE EMISIÓN'), $fontStyle, 'p2Style');
$table_des->addCell(1500, $styleCell)->addText('CLIENTE', $fontStyle, 'p2Style');
...
// CONSULTA PARA SABE SI HAY INCREMENTO POR LEVANTAMIENTO //
...
while ($row3 = $obj_bd->FuncionFetch($resultado3)) {
    ...
    $table_act->addCell(1800, $styleCell)->addText(utf8_decode('Responsable Area Civil'), $fontStyle, 'p2Style');
    $table_act->addCell(1000, $styleCell)->addText(utf8_decode('Responsable Area Eléctrica'), $fontStyle, 'p2Style');
    $table_act->addCell(1000, $styleCell)->addText(utf8_decode('Responsable Area Mecánica'), $fontStyle, 'p2Style');
    $table_act->addCell(1000, $styleCell)->addText(utf8_decode('Responsable Area LLTT'), $fontStyle, 'p2Style');
    ...
    // ALCANCE BAREMADO //
    $section->addText(utf8_decode('ALCANCE BAREMADO: '), $fontStyle);
    $obs = utf8_encode($row3['presupuesto_obs']);
    $section->addText(utf8_decode($obs), $fontStyle_texto, $paragraphOptions);
    $section->addTextBreak(1);

    // ENTREGABLES //
    $section->addText(utf8_decode('ENTREGABLES: '), $fontStyle);
    $obs = utf8_encode($row3['presupuesto_obs']);
    $section->addText(utf8_decode($obs), $fontStyle_texto, $paragraphOptions);
    $section->addTextBreak(1);
    ...
    // NOMBRE RESPONSABLE CUADRILLA //
    $section->addText(utf8_decode('NOMBRE RESPONSABLE CUADRILLA: '), $fontStyle);
    $obs = utf8_encode($row3['presupuesto_obs']);
    $section->addText(utf8_decode($obs), $fontStyle_texto, $paragraphOptions);
    $section->addTextBreak(1);

    // NOMBRE COORDINADOR //
    $section->addText(utf8_decode('NOMBRE COORDINADOR: '), $fontStyle);
    $obs = utf8_encode($row3['presupuesto_obs']);
    $section->addText(utf8_decode($obs), $fontStyle_texto, $paragraphOptions);
    $section->addTextBreak(1);
}

// CONSULTA LEVANTAMIENTO //
$sql_incremento = "SELECT * 
                    FROM dt_incremento_presupuesto 
                    WHERE detallepresupuesto_id=$det_pret
                    AND incrementopresupuesto_estado=1";
...
if ($existe_incremmentos > 0) {
    ...
    if ($row6["incrementopresupuesto_tipo"] == "Actividades de Levantamento") { 
        ...
        // G. Plan de calidad:
        $section->addTextBreak(1);
        $section->addText(utf8_decode('H. Normatividad:'), $fontStyle1);
        $section->addText(utf8_decode('El contratista elaborará los planes de calidad de acuerdo con lo dispuesto en el Sistema de Gestión de calidad (SGS) vigente de CODENSA S.A. ESP'), $fontStyle_texto, $paragraphOptions);
    }else{
    
        //F. Plan de calidad:
        $section->addTextBreak(1);
        $section->addText(utf8_decode('G. Normatividad:'), $fontStyle1);
        $section->addText(utf8_decode('El contratista elaborará los planes de calidad de acuerdo con lo dispuesto en el Sistema de Gestión de calidad (SGS) vigente de CODENSA S.A. ESP'), $fontStyle_texto, $paragraphOptions);
    }
}
...
//tabla firmas
$gestor_sql = "SELECT CONCAT(usu.usuario_nombre,' ',usu.usuario_apellidos) AS gestor, 
                        dp.detallepresupuesto_codensaGestor
                FROM dt_detalle_presupuesto dp
...
COORDINADOR AREA CIVIL") . "", $fontStyle_texto, $firmas);
...
$table_firm->addCell(6000)->addText("_______________________________________\n" .
        utf8_decode("Ing.                                      
COORDINADOR AREA MECÁNICA") . "", $fontStyle_texto, $firmas);
...
$table_firm->addCell(6000)->addText("_______________________________________\n" .
        utf8_decode("Ing.                                       
SUPERVISOR DE REVISIÓN") . "", $fontStyle_texto, $firmas);
