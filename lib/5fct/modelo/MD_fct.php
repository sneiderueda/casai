...
class MD_fct {
        ...
        $(function() {
           $('#txtInicioFactura').datetimepicker({                        
               ...
               $('#txtFinFactura').datetimepicker({
                   ...
                   <button type='button' class='btn btn-success' id='btn_generar' value='Generar Consolidado' onclick='GenerarFactura(1)'><img src='img/report_excel.png' width='20' height='20'> Generar Consolidado</button>
                   </center>
                   </td>
                   </tr>
	...
	public function ListActividadesAfacturar($data) {
		...
		/////////////////////////////////////////////////////////////////////////////////
          ...
          $row_pend = $obj_bd->FuncionFetch($res_exi);
          ...
          if ($filas > 0) {
	          	...
	          	if ($row_pend['factura_porcentajependiente'] != 0) {
            		...
            		$tabla .= "<td><input type='text' style='text-align:center' id='porc_sub_" . $row['presupuesto_id'] . "' name='porc_sub_" . $row['presupuesto_id'] . "' maxlength='6' value='" . $row_pend['factura_porcentajependiente'] . "' placeholder='Numero' style='width:60px' class='a_txt_porc ing_datos' onkeypress='return decimales(event)' onblur='CalPorcConciliacion(this.value," . $row['presupuesto_id'] . "," . $row['actividad_valorservicio'] . ", ".$new_acta.",".$row_pend['factura_porcentajependiente'].");'></td>
        			...
        			<td><input type='button' id='consolidar_" . $row['presupuesto_id'] . "' value='Consolidar' class='btn btn-warning' style = '' onclick='actualizar_conciliacion(" . $row['presupuesto_id'] . ",".$cantidad.",".$valor_porcentaje.",".$new_acta.",".$row_pend['factura_id'].");'></td>";
            	}else{

            		$valor_porcentaje = 0;

            		$tabla .= "<td><input type='text' style='text-align:center' id='porc_sub_" . $row['presupuesto_id'] . "' name='porc_sub_" . $row['presupuesto_id'] . "' maxlength='6' value='" . $row_pend['factura_porcentajependiente']. "' placeholder='Numero' style='width:60px' class='a_txt_porc ing_datos' onkeypress='return decimales(event)' disabled='true' onblur='CalPorcConciliacion(this.value," . $row['presupuesto_id'] . "," . $row['actividad_valorservicio'] . ", ".$new_acta.",".'1'.");'></td>
          	...
          	$tabla .= "<td><input type='text' style='text-align:center' id='porc_sub_" . $row['presupuesto_id'] . "' name='porc_sub_" . $row['presupuesto_id'] . "' maxlength='6' value='" . $cantidad . "' placeholder='Numero' style='width:60px' class='a_txt_porc ing_datos' onkeypress='return decimales(event)' onblur='CalPorcConciliacion(this.value," . $row['presupuesto_id'] . "," . $row['actividad_valorservicio'] . ", ".$new_acta.",".$cantidad.");'></td>
          
           <td><input type='hidden' style='text-align:center' id='valor_cal_sub_" . $row['presupuesto_id'] . "' name='valor_cal_sub_" . $row['presupuesto_id'] . "' value='" . round($valor_porcentaje, 0) . "'  style='width:100px' disabled='disabled' class='input-medium a_valor_cal'>
				<input type='text' style='text-align:center' id='valor_cal_sub1_" . $row['presupuesto_id'] . "' name='valor_cal_sub_" . $row['presupuesto_id'] . "' value='" . number_format((float)$valor_porcentaje, 0, ',', '.') . "'  style='width:100px' disabled='disabled' class='input-medium a_valor_cal'>
           </td>

            <td><input type='button' id='consolidar_" . $row['presupuesto_id'] . "' value='Consolidar' class='btn btn-primary' onclick='agregar_conciliacion(" . $row['presupuesto_id'] . ",".$cantidad.",".$valor_porcentaje.",".$new_acta.");'></td>";
          }
       }
    ...
    function ListCerradas() {
        ...
        <th class="text-center">Subestación</th>  
        ...
        // CONSULTAS //
        ///////////////
		$sql = "CALL SP_facturacion('4','','','','','','','','','','','','','','','','','','','','','');";

        $consulta = $obj_bd->EjecutaConsulta($sql);
	    while($conci = $obj_bd->FuncionFetch($consulta))
	    {
	        $tabla .= 	"<tr>
						<td class='text-uppercase text-center'><button class='btn btn-info fondo' value='".$conci['ordentrabajo_id'] ."' onclick='listar_conciliacion(".$conci['ordentrabajo_id'] .");'>".$conci['ordentrabajo_num'] ."</button></td>
						<td class='text-uppercase'>".utf8_encode($conci['subestacion_nombre']) ."</td>
						<td class='text-uppercase'>".utf8_encode($conci['detallepresupuesto_nombre']) ."</td>
						<td class='text-uppercase'>".utf8_encode($conci['nombre_gestor']) ."</td>
	        			</tr>";
	    }    
    ...
    */
        ...
        if ($cuenta == 2){
            ...
            if($desc == "DISEÑO"){
                ...
                */
                ...
                $sql = "CALL SP_facturacion('1','','".$iva."','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','".$porcentaje."','".$cantidad."','".$porcentaje_pendiente."','".$id_usuario."','','".$valor_labor."','".$valor_pendiente."','".$valor_subtotal."','".$presupuesto_id."','".$ot_id."','".$acta."');";
               ...
               $sql = "CALL SP_factura('10','','','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','0','0','0','".$id_usuario."','','0','0','0','".$presupuesto_id."','".$ot_id."','".$acta."');";

               $consulta = $obj_bd->EjecutaConsulta($sql);
               $row2 = $obj_bd->FuncionFetch($consulta);

               $sql = "CALL SP_factura('10','','','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','".$porcentaje."','".$cantidad."','".$porcentaje_pendiente."','".$id_usuario."','','".$valor_labor."','".$valor_pendiente."','".$valor_subtotal."','".$presupuesto_id1."','".$ot_id."','".$acta."');";
                ...
                */
                ...
                $sql = "CALL SP_factura('10','','','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','".$porcentaje."','".$cantidad."','".$porcentaje_pendiente."','".$id_usuario."','','".$valor_labor."','".$valor_pendiente."','".$valor_subtotal."','".$presupuesto_id."','".$ot_id."','".$acta."');";
    ...
    function actualizar_conciliacion($post){
        ...
        $sql_exi = "call SP_facturacion('3', '', '', '', '', '', '', '', '" . $fechaFacturaMes . "', '" . $fechaFacturaFin . "', '', '', '', '', '', '', '', '', '', '" . $presupuesto_id . "', '".$ot_id."', '');";
        ...
        $acta = $acta + 1;
        $porcentaje_pendiente = $row_pend['factura_porcentajependiente'] - $porcentaje;
        $valor_pendiente = round($row_pend['factura_valorpendiente'] - $valor_labor);
        ...
        if ($cuenta == 2){
            ...
            if($desc == "DISEÑO"){
                ...
                */
                ...
                $sql = "CALL SP_facturacion('1','','".$iva."','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','".$porcentaje."','".$row_pend['factura_porcentajependiente']."','".$porcentaje_pendiente."','".$id_usuario."','','".$valor_labor."','".$valor_pendiente."','".$row_pend['factura_valorpendiente']."','".$presupuesto_id."','".$ot_id."','".$acta."');";
               ...
               $sql = "CALL SP_factura('10','','','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','".$porcentaje."','".$row_pend['factura_porcentajependiente']."','".$porcentaje_pendiente."','".$id_usuario."','','".$valor_labor."','".$valor_pendiente."','".$row_pend['factura_valorpendiente']."','".$presupuesto_id1."','".$ot_id."','".$acta."');";
                ...
                */
                ...
                $sql = "CALL SP_factura('10','','','','','','','','".$fechaFacturaMes."','".$fechaFacturaFin."','0','".$porcentaje."','".$row_pend['factura_porcentajependiente']."','".$porcentaje_pendiente."','".$id_usuario."','','".$valor_labor."','".$valor_pendiente."','".$row_pend['factura_valorpendiente']."','".$presupuesto_id."','".$ot_id."','".$acta."');";
    ...
    public function listar_conciliacion($post)
    {
	    /////////////
    	// OBJETOS //
	    /////////////
        ...
        $tabla .= "<br><br><br><br>";
        $tabla .= "<fieldset class='letraBl'>";
        $tabla .= "<legend class='titulo'><span>Actas Consolidadas</span></legend>";
        $tabla .= "<br>";
        $tabla .= '<div class="table-responsive">';
        $tabla .= '<table cellpadding="0" class="table table-bordered table-hover" cellspacing="0" border="0" id="example">
        <thead>
        <tr><button class="btn btn-default" onclick="ListCerradas()">Regresar</button></tr>
        <tr class="fondo letraN">                              
        <th class="text-center">No. Acta</th>  
        <th class="text-center">Subtotal</th>  
        <th class="text-center">Inc. Ubicación</th>
        <th class="text-center">Inc. 90 días</th>
        <th class="text-center">Subtotal + Incrementos</th>
        <th class="text-center">Iva</th>
        <th class="text-center">Total + Iva</th>
        <th class="text-center">Fecha Conformidad</th>
        <th class="text-center">No. Conformidad</th>
        <th class="text-center">Valor Conformidad</th>
	    ...
	    // CONSULTAS //
	    ///////////////
	    $sql = "CALL SP_facturacion('5','','','','','','','','','','','','','','','','','','','','".$ot_id."','');";

        $consulta = $obj_bd->EjecutaConsulta($sql);

        while($row_acta = $obj_bd->FuncionFetch($consulta))
		{
			$diferencia = $row_acta['factura_diferencia'];

			/////////////////////////////
			// CALCULAMOS LA UBICACION //
			/////////////////////////////
			$sql1 = "CALL SP_facturacion('6','','','','','','','','','','','','','','','','','','','','".$ot_id."','".$row_acta['factura_actanum']."');";
			$consulta1 = $obj_bd->EjecutaConsulta($sql1);
			$row_ubicacion = $obj_bd->FuncionFetch($consulta1);

			$ubicacion = round($row_ubicacion['ubicacion'],0);
			...
			// CALCULOS //
			//////////////
			$sub_ubi = $row_acta['subtotal'] + $ubicacion;
			$dias = round($sub_ubi*0.015);
			$total = round($sub_ubi + $dias);
			$iva = round(($total*$row_acta['factura_iva'])/100);
			$total_iva = round($total + $iva);

			//////////////////
			// MOSTRAR INFO //
			//////////////////
			$tabla .= 	"<tr>
							<td class='text-center'>Acta ".$row_acta['factura_actanum']."</td>
							<td>$".number_format($row_acta['subtotal'],0,',','.')."</td>
							<td>$".number_format($ubicacion,0,',','.')."</td>
			...
			if ($diferencia != 0) 
			{
		        ...
		        $tabla .= "<script>
						        $(function() {
						           $('#fecha_conformidad_".$row_acta['factura_actanum']."').datetimepicker({                        
						               format: 'YYYY-MM-DD',
						               minDate: '01-01-2017',                        
						               changeMonth: true,
						               changeYear: true                               
				...
				<td colspan='1' class='text-center'><div class='input-group date ' id='InicioSeg'  style='width:100px'> <input type='text' placeholder='yyyy/mm/dd' id='fecha_conformidad_".$row_acta['factura_actanum']."' name='txtInicioFactura' class='form-control data text-center' readonly required='true' /></div></td>";
				
				$tabla .= "<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='num_conformidad_".$row_acta['factura_actanum']."' placeholder='Número' value='".$row_acta['factura_numero_conformidad']."'/></td>
				<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='valor_conformidad_".$row_acta['factura_actanum']."' placeholder='Valor' onblur='cal_diferencia(this.value," . $diferencia . "," . $row_acta['factura_actanum'] . ");'/></td>
				<td class='text-center'><input type='text'class='text-center bg-danger form-control' style='width:100px' disabled id='diferencia_".$row_acta['factura_actanum']."' value='".$diferencia."'/></td>
				<td class='text-center'><button class='btn btn-warning' id='guardar_".$row_acta['factura_actanum']."' onclick='guardar_conformidad(".$row_acta['factura_actanum'].",".$ot_id.",".$row_acta['factura_id'].");'>Guardar</button></td>
				</tr>";
				...
				$tabla .= "<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='num_conformidad_".$row_acta['factura_actanum']."' placeholder='Número' value='".$row_acta['factura_numero_conformidad']."'/></td>
				<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='valor_conformidad_".$row_acta['factura_actanum']."' placeholder='Valor' onblur='cal_diferencia(this.value," . $total . "," . $row_acta['factura_actanum'] . ");'/></td>
				<td class='text-center'><input type='text'class='text-center form-control' style='width:100px' disabled id='diferencia_".$row_acta['factura_actanum']."' value='".$total."'/></td>
				<td class='text-center'><button class='btn btn-primary' id='guardar_".$row_acta['factura_actanum']."' onclick='guardar_conformidad(".$row_acta['factura_actanum'].",".$ot_id.",".$row_acta['factura_id'].");'>Guardar</button></td>
				</tr>";
			}
		}
    ...
    public function guardar_conformidad($post)
    {
    	/////////////
    	// OBJETOS //
	    /////////////
	    ...
	    $ot_id = $post['ot_id'];
	    $acta = $post['acta'];
	    $num_conformidad = $post['num_conformidad'];
	    $fecha_conformidad = $post['fecha_conformidad'];
	    $valor_conformidad = $post['valor_conformidad'];
	    $diferencia = $post['diferencia'];

	    // return "hola soy ".$fecha_conformidad;

	    //////////////
	    // CONSULTA //
	    //////////////
	    $query_conf = "CALL SP_facturacion('7','".$fac_id."','".$fecha_conformidad."','','','','','','','','".$num_conformidad."','".$valor_conformidad."','".$diferencia."','','','".$id_usuario."','','','','','".$ot_id."','".$acta."');";
		$res_query = $obj_bd->EjecutaConsulta($query_conf);
    ...
    public function buscar_conformidad($post)
    {
    	$tabla = "";

        $tabla .= "<br><br><br><br>";
        $tabla .= "<fieldset class='letraBl'>";
        $tabla .= "<legend class='titulo'>Buscar Conformidad</legend>";

        $tabla .= "<br>";
        $tabla .= '	<div class="form-group">
            			<label for="conformidad" class="col-sm-2 control-label h4">No. Conformidad:</label>
            			<div class="col-sm-2">                
                			<input type="number" class="form-control text_center" id="conformidad">
            			</div>
            			<button class="btn btn-info" onclick="listar_conformidad(0);"><i class="fa fa-search"></i> Buscar</button>
        			</div>';
        $tabla .= "<fieldset>";
        return $tabla;
    }

    public function listar_conformidad($post)
    {
    	/////////////
    	// OBJETOS //
	    /////////////
        ...
        $tabla .= "<br>";
        $tabla .= "<fieldset class='letraBl'>";
        // $tabla .= "<legend class='titulo'><span>Conformidad No. ".$num."</span></legend>";
        ...
        if ($opc == 1) 
        {
		    ...
		    while ($row_conf = $obj_bd->FuncionFetch($res_conformidad))
		    {
	        	...
	        	$tabla .= "<script>
						        $(function() {
						           $('#fecha_radicado_".$row_conf['factura_actanum']."').datetimepicker({                        
						               format: 'YYYY-MM-DD',
						               minDate: '01-01-2017',                        
						               changeMonth: true,
						               changeYear: true                               
		        ...
		        if ($row_conf['conformidad_factura'] > 0) {
		        	$tabla .= "
						<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='num_factura_".$row_conf['factura_actanum']."' placeholder='Número' disabled='disabled' value='".$row_conf['conformidad_factura']."'/></td>

						<td colspan='1' class='text-center'><div class='input-group date' id='InicioSeg'  style='width:100px'> <input type='text' placeholder='yyyy/mm/dd' id='fecha_radicado_".$row_conf['factura_actanum']."' value='".$row_conf['conformidad_fecharadicado']."' disabled='disabled' class='form-control data text-center' readonly required='true' /></div></td>
						
						<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='num_radicado_".$row_conf['factura_actanum']."' placeholder='Número' disabled='disabled' value='".$row_conf['conformidad_radicado']."'/></td>

						<td class='text-center'><button class='btn btn-primary' id='guardar_".$row_conf['factura_actanum']."' style='display:none;' onclick='guardar_radicado(".$row_conf['factura_actanum'].",".$row_conf['conformidad_num'].");'>Guardar</button></td>
						</tr>";	
		        }else{

					$tabla .= "
						<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='num_factura_".$row_conf['factura_actanum']."' placeholder='Número' value='".$row_conf['factura_numero_conformidad']."'/></td>

						<td colspan='1' class='text-center'><div class='input-group date' id='InicioSeg'  style='width:100px'> <input type='text' placeholder='yyyy/mm/dd' id='fecha_radicado_".$row_conf['factura_actanum']."' name='txtInicioFactura' class='form-control data text-center' readonly required='true' /></div></td>
						
						<td class='text-center'><input type='number'class='text-center form-control' style='width:100px' id='num_radicado_".$row_conf['factura_actanum']."' placeholder='Número'/></td>

						<td class='text-center'><button class='btn btn-primary' id='guardar_".$row_conf['factura_actanum']."' onclick='guardar_radicado(".$row_conf['factura_actanum'].",".$row_conf['conformidad_num'].");'>Guardar</button></td>
						</tr>";
		        }
			...
			$tabla .= "	<legend class='titulo'><span class='col-sm-6'>No. Conformidad: $ ".$num."</span><span>Total Conformidad: $ ".number_format($suma_conformidad,0,'.','.')."</span></legend>
						</tbody>
	        			</table>
	        			</div>
	        			<script>$('#example').DataTable();</script>";
    ...
    public function buscar_conf_fact($post)
    {
    	$opc = "1";
    	$tabla = "";
        ...
        $tabla .= '	<div class="form-group">
            			...
            			<button class="btn btn-info" onclick="listar_conformidad('.$opc.');"><i class="fa fa-search"></i> Buscar</button>
        			</div>';
        $tabla .= "<fieldset>";
    ...
    public function guardar_radicado($post)
    {
	    /////////////
    	// OBJETOS //
	    /////////////
    	...
    	$num_radicado = $post['num_radicado'];

	    //////////////
    	// CONSULTA //
	    //////////////
    	$query_conformidad = "CALL SP_conformidad ('2','','','','','".$num."','','','".$num_factura."','".$num_radicado."','".$fecha_radicado."','')";
	    $res_conformidad = $obj_bd->EjecutaConsulta($query_conformidad);
