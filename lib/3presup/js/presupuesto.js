...
function SavePresupuesto() {
    ...
    var txt_otAnteriores = $('#txt_otAnteriores').val();
    ...
    $.ajax({
        ...
        data: {
            ...
            'txt_otAnteriores' : txt_otAnteriores
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert('Error (Petición) { ' + jqXHR + ' - ' + textStatus + ' - ' + errorThrown + ' }');
        },
    ...
    $.ajax({
        ...
        success: function (retu) {
            if (retu == "0") {
                ...
                $("#agregar_labor").css("display", "block");
...
function CalValorPorcPresupuestoSub(val_porc, detalleactividad_id, actividad_valor, detallesubactividad_porc) {
    ...
    // var formato = valor.toFixed(0);
    // var formato= valor.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
    var punt = number_format(valor, 0, ',', '.');

    console.log(valor);
    $("#valor_cal_sub_" + detalleactividad_id).val(valor);
    $("#valor_cal_sub1_" + detalleactividad_id).val(punt);
    $("#valor_string" + detalleactividad_id).val(valor);
...
function SaveActividadPresupuesto(baremo_id) {
    ...
    var check = $('#check').val();
    var incremento_ubicacion = $('#incremento_ubicacion').val();
    var incremento_90dias = $('#incremento_90dias').val();
    var totalIncrementos = $('#totalIncrementos').val();
    ...
    var parametros = {
        ...
        'incremento_90dias': incremento_90dias,
        'incremento_ubicacion': incremento_ubicacion,
        'check': check
    };
...
function JsonDetallePresupuesto(detallepresupuesto_id, control) {
    ...
    if (control == '1') {
        ...
        $("#txt_total").val(jsondata.detallepresupuesto_total_formato);
        ...
        $('#txt_otAnteriores').val(jsondata.anterioresot_descripcion);
        ...
        contar_labores(detallepresupuesto_id);
    }
...
function contar_labores(detallepresupuesto_id) 
{
    $.ajax({
        type: "POST",
        url: 'lib/3presup/controlador/CT_presup.php',
...
/*****************************************************************************/
function check() { //inicio

    // se lee el documento html
    $(document).ready(function () {
        // se lee el cambio del checkbox
        $('#checkboxUbicacion').change(function () {
            // se define el estado del checkbox
            if ($(this).is(":checked")) {
                $('#check').val(1);
                calcularIncrementos();
            } else {
...
/*****************************************************************************/
function calcularIncrementos() { //inicio funcion
    ...
    // se definen parametros para enviar por ajax
    var parametros = {
        'detallepresupuesto_id': detallepresupuesto_id,
        'check': check,
        'opcion': 'calcularIncrementos'
    };
    ...
    // se envia el resultado a html
    $("#incremento_ubicacion").val(number_format(jsondata.ubicacion, 0, ',', '.'));
    $("#incremento_90dias").val(number_format(jsondata.dias, 0, ',', '.'));
    $("#totalIncrementos").val(number_format(jsondata.total, 0, ',', '.'));
    //alert(check);

} //fin funcion

/*****************************************************************************/
function guardarIncrementos() { //inicio funcion
...
/*****************************************************************************/
function agregarDocumento() { //inicio
    ...
    $.ajax({
        type: "POST",
        url: 'lib/3presup/view/formAgregarDocumento.php',
        data: {
            detallepresupuesto_id: detallepresupuesto_id
        },
        async: false,
...
} // fin agregarDocumentos

function CalPorcConciliacion(val_porc, detalleactividad_id, actividad_valor, detallesubactividad_porc,cantidad) {

    if (val_porc > cantidad) {

        alert("El valor consolidado no puede exceder "+ cantidad);
        $("#porc_sub_"+detalleactividad_id).val(cantidad);
